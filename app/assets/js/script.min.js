var adriftApp=angular.module("adriftApp",["ngRoute","customFilters","userControllers","topicControllers"]);adriftApp.config(["$routeProvider","$locationProvider",function($routeProvider,$locationProvider){$locationProvider.html5Mode({enabled:true});$routeProvider.when("/adrift/locations",{templateUrl:"partials/topics/index.html",controller:"TopicIndexCtrl"}).when("/adrift/settings",{templateUrl:"partials/users/settings.html",controller:"UserSettingsCtrl"}).when("/adrift/collection",{templateUrl:"partials/users/collection.html",controller:"UserCollectionCtrl"}).when("/adrift/locations/:topicName",{templateUrl:"partials/topics/show.html",controller:"TopicShowCtrl"}).otherwise({redirectTo:"/adrift/locations"})}]);adriftApp.controller("MessageRequestCtrl",["$scope","$http","$interval",function($scope,$http,$interval){angular.element(document).ready(function(){$scope.requestMessage=function(){$http.get("/messages").success(function(data){console.log("Getting Messages...")})};$interval($scope.requestMessage,1e3)})}]);angular.module("customFilters",[]).filter("capitalize",function(){return function(input,scope){if(input!=null)input=input.toLowerCase();return input.substring(0,1).toUpperCase()+input.substring(1)}});// Topic Controllers
var topicControllers=angular.module("topicControllers",[]);topicControllers.controller("TopicIndexCtrl",["$scope","$http",function($scope,$http){$http.get("topics/").success(function(data){$scope.topics=data.topics})}]);topicControllers.controller("TopicShowCtrl",["$scope","$http","$routeParams",function($scope,$http,$routeParams){$http.get("topics/"+$routeParams.topicName).success(function(data){$scope.topic=data.topic})}]);topicControllers.controller("TopicSubscribeCtrl",["$scope","$http","$routeParams",function($scope,$http,$routeParams){$http.get("users/current").success(function(data){$scope.currentUser=data.currentUser;if($scope.currentUser.topics.indexOf($routeParams.topicName)!=-1){$scope.subscription="Subscribed";$scope.subscribed=true}else{$scope.subscription="Subscribe";$scope.subscribed=false}$scope.locationSubscribe=function(){if($scope.subscribed==false){$http.patch("users/topics/"+$routeParams.topicName).success(function(data,status){console.log(data)});$scope.subscription="Subscribed";$scope.subscribed=true}else if($scope.subscribed==true){$http.delete("users/topics/"+$routeParams.topicName).success(function(data,status){console.log(data)});$scope.subscription="Subscribe";$scope.subscribed=false}}})}]);topicControllers.controller("TopicMessageCtrl",["$scope","$http","$routeParams",function($scope,$http,$routeParams){$scope.message={topicName:$routeParams.topicName};$scope.addMessage=function(message){$scope.messageCopy=angular.copy($scope.message);$http.post("messages/",{message:$scope.messageCopy}).success(function(data,status){$scope.status=status;$scope.data=data;$scope.result=data});$scope.message.content=""}}]);// User Controllers
